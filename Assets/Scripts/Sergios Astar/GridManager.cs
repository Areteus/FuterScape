...
public class GridManager : MonoBehaviour
{
    ...
    {
        nodes = new SNode[numberOfColumns, numberOfRows];
        ...
        for (int i = 0; i < numberOfColumns; i++) 
        {
            for (int j = 0; j < numberOfRows; j++) 
            {
                ...
                SNode node = new SNode(cellPosition);
                nodes[i, j] = node;
    ...
    public void GetNeighbors(SNode node, ArrayList neighbors)
    {
    ...
    private void AssignNeighbor(int row, int column, ArrayList neighbors)
    {
        ...
        {
            SNode nodeToAdd = nodes[row, column];
